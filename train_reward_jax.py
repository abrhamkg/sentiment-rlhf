...
# TODO: Ensure torch CPU is used to avoid conflicts with jax
...
class Args:
    ...
    # TODO: switch base model to tiny-gpt2
    base_model: str = "sshleifer/tiny-gpt2"
    """the name of the pretrained model to use"""
    # label_dataset: str = "sentiment/offline_5k.json"
    label_dataset: str = "hh-rlhf-processed/sentiment-base-combined.json"
    ...
    local_rollout_batch_size: int = 32
...
class RewardModelParams:
    ...
    # TODO: Add __iter__ method to make class iterable
    def __iter__(self):
...
class NormalizationDataset(IterableDataset):
    ...
    def __iter__(self):
        for text in self.generator("train", self.seed, shuffle=True):
            ...
            yield output["input_ids"].astype(np.int32)
...
def create_initial_reward_state_and_models(init_key, args):
    ...
    if args.precision == 'half':
        lm_backbone.params = lm_backbone.to_bf16(lm_backbone.params)
    ...
    # params_dict = flax.core.FrozenDict(params)
...
# TODO: Convert tensors to float16 to match model
def prepare_left_padded_query_responses_with_labels(dataset, args, dtype=np.int32):
    ...
    labels = np.array(dataset["best"], dtype=dtype)
    # [num_queires,]

    queries = np.stack(dataset["query"], dtype=dtype)  # N.B only works because all queries have the same length of 64
    # [num_queires, max_query_length]

    queries = np.repeat(queries, args.labels.num_labels, axis=0)  # [num_queries * num_response_per_query, max_query_len]
    queries = rearrange(queries, "(q r) l -> q r l", r=args.labels.num_labels)
    # [num_queires, num_response_per_query, max_query_length]

    responses = np.array([np.stack(dataset[f"sample{i}"])
                          for i in range(args.labels.num_labels)], dtype=dtype)
...
def train(args: Args):
    ...
    untrained_model = FlaxAutoModelForCausalLM.from_pretrained(args.base_model, dtype=jnp.float16)
    if args.precision == 'half':
        untrained_model.params = untrained_model.to_bf16(untrained_model.params)
    ...
    # )["train"]  # 6264 entries with 6 features ('best', sample{0-3}, 'query')

    labeled_dataset = load_dataset(
        "json",
        data_files={'train':args.label_dataset, 'test': 'hh-rlhf-processed/test.json'},
    )  # 6264 entries with 6 features ('best', sample{0-3}, 'query')
    train_dataset = labeled_dataset['train']
    test_dataset = labeled_dataset['test']

    print("Num labels found in source:", len(train_dataset))
    print("training on", len(train_dataset), "in batches of", args.local_batch_size)

    all_queries_responses, all_labels = prepare_left_padded_query_responses_with_labels(train_dataset, args)
    test_queries_responses, test_labels = prepare_left_padded_query_responses_with_labels(test_dataset, args)

    assert args.labels.num_train < all_queries_responses.shape[0]
    train_queries_responses = all_queries_responses
    train_labels = all_labels

    val_queries_responses = test_queries_responses
    ...
    for global_step, train_batch in enumerate(train_iter):
        ...
        # (#devices, local_batch_size, #comparisons, seq_len), (#devices, batch_size)
    ...
    if args.save_path and args.local_rank == 0:
        ...
        save_dir = os.path.join(os.path.abspath(os.curdir), args.save_path)
        orbax_checkpointer.save(save_dir, ckpt, save_args=save_args, force=True)
